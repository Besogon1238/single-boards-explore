...
* Перед попыткой подключения стоит убедиться, что на целевой плате установлен gcc и его бибилиотеки. 
...
Суть проблемы следующая. При успешном подключение по ssh по пути /$HOMEDIR/.vscodium-server/bin/ устанавливается баш скрипт, который выкачивает [отсюда](https://github.com/VSCodium/vscodium/releases/download/1.99.32562/vscodium-reh-linux-riscv64-1.99.32562.tar.gz) вот этот вот исполняемый файл vscode-server и разрешает его запускать. Скрипт приведен в описании к этому [issue](https://github.com/VSCodium/vscodium/issues/2232) пользователем Ysnv1997.

Я руками поставил архив, распаковал куда надо и запустил скрипт по пути /$HOMEDIR/.vscodium-server/bin/1f9bbd5db96ffb6200c7732948af7aeb7d0773ae/bin/codium-server. После этого подключение с хостовой машины будет работать.

После этого отлаживать код на Lichee RV и Mango PI станет возможным, но с кокретными ограничениями в производительности. Скомпилировать и запустить Hello-World программу можно приблизительно за 20~ секунд.
